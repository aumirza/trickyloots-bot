...
def pre_process(message):	
    ...
    if any( re.match(blockmessage, message, re.IGNORECASE) for blockmessage in blockmessages):	
        return None	
    else:
        msg_list = []	
        for line in message.splitlines():	
            if any( re.match(blockline,line, re.IGNORECASE) for blockline in blocklines):	
                pass
            else:
                if any(re.match(blockword,line, re.IGNORECASE) for blockword in blockwords):		
...
def in_conveter(domain, url):
    ...
    if domain in flipkart:
        if domain == flipkart[0]:  # if url is of type dl.flipkart.com
